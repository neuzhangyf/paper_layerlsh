...
To deal with the dynamic data, it is straightforward to make LayerLSH support point insert or delete operations. Algorithm \ref{alg:split} can be accordingly changed for updating the index structure, where the input is the inserted or deleted points. A point is inserted into or deleted from a specified bucket followed by checking the bucket size. Once the bucket size is bigger than $T_u$, the bucket is split. Once the bucket size is smaller than $T_l$, it is marked as underloaded, and several similar underloaded buckets will be searched together when query processing.
