...
\Input{new point $p$, LayerLSH tables $HT=\{HT_1,\ldots,HT_l\}$, maximum tolerance $\epsilon_m$, caching tolerance $\epsilon_c$, time window $W$}
...
\While{true}{
    ...
    \If{$t_c-t_l\geq W$}{
        recursively split $HT$ according to $(1+\epsilon_c)\cdot T_u$\;
        $t_l\leftarrow$ current time;
    }
    \StreamInsert($p$, $HT$)\;
}
\BlankLine
\Fn{\StreamInsert($p$, $HT$)}{\
    \ForEach{$HT_i$ in $HT$}{
        ...
        \If{$b$ is a pointer bucket}{
            $HT_{child}\leftarrow$ locate the child hash tables $b$ points to\;
            status$\leftarrow$\StreamInsert($p$, $HT_{child}$, $R'$, $P'$)\;
            \If{status$==$overloaded}{
                $HT_{child}\leftarrow$ resplit $HT_{child}$ according to the table size);
            }
        }
        \ElseIf{$b$ is a data bucket}{
...
\caption{Streamed Insertions in LayerLSH}
\label{alg:lshquery}
\end{algorithm}
